...
| 暗号化方式 | シナリオ | なぜこの方式か？ | 身近な例 |
| :--- | :--- | :--- | :--- |
| **共通鍵暗号方式** | **個人のPC内でのファイルやフォルダの保護**<br>（自分しか使わない機密ファイルの保管） | ・鍵を他者と安全に交換する必要がない。<br>・処理が高速で大容量ファイルに適している。 | ・パスワード付きZIPファイル<br>・ディスク暗号化ツール (VeraCrypt) |
| | **アプリケーションとDB間のデータ保護**<br>（DB内の個人情報などを暗号化） | ・通信相手が特定されているため鍵の管理が容易。<br>・頻繁なデータの読み書きに高速な処理が求められる。 | ・データベースの特定列の暗号化（透過的データ暗号化） |
| **公開鍵暗号方式** | **オンラインショッピングサイトでの通信**<br>（HTTPS/SSL/TLSによる通信経路の暗号化） | ・不特定多数の相手と安全な通信を開始できる。<br>・安全に「共通鍵」を交換するために使われる（ハイブリッド暗号）。 | ・ブラウザのURL欄の「鍵マーク」<br>・ECサイトでの決済画面 |
| | **電子契約サービスでの電子署名**<br>（本人証明と改ざん防止） | ・秘密鍵で署名し、公開鍵で検証することで本人性と非改ざん性を証明できる。<br>・なりすましや契約内容の変更を防ぐ。 | ・電子契約サービス (クラウドサインなど)<br>・マイナンバーカードを使ったe-Tax |
| **ハッシュ化** | **Webサービスにおけるパスワードの保存**<br>（ログインパスワードの安全な保管） | ・不可逆（元に戻せない）なため、万が一データが漏洩しても元のパスワードが分からない。<br>・ログイン時は入力値のハッシュを比較して認証する。 | ・ほぼ全てのWebサイトやアプリのログイン機能 |
| | **ソフトウェアのダウンロードファイルの検証**<br>（ファイルが本物で壊れていないか確認） | ・データが1ビットでも異なればハッシュ値が全く異なるため、改ざんや破損を検知できる。<br>・元のデータ内容を知る必要なく同一性を確認できる。 | ・ソフトウェア配布サイトのチェックサム (SHA-256など) |

### 種別

| 暗号化方式 | 目的 (保護対象) | 暗号化のタイミング | 鍵の管理者 | クラウド側の可視性 | 主な利用技術 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **転送時の暗号化**<br>(In-Transit Encryption) | 通信経路上でのデータの保護<br>（クライアントとサーバー間の通信） | クライアントとサーバーの**通信中** | 通信セッション毎に一時的な鍵を生成・管理 | サーバー到着後は復号されるため、平文を処理できる | TLS (HTTPS), SSL |
| **サーバーサイド暗号化**<br>(Server-Side Encryption) | 保管中（At-Rest）のデータの保護<br>（ストレージやDB上のデータ） | データがサーバーに**到着した直後** | **クラウド事業者**<br>（またはKMS等で共同管理） | 暗号化/復号処理時に平文にアクセス可能 | AES256, Cloud KMS, AWS KMS |
| **クライアントサイド暗号化**<br>(Client-Side Encryption) | 保管中（At-Rest）のデータの保護<br>（最高レベルの機密性） | データがクライアントから**送信される前** | **ユーザー自身** | 常に暗号化されており、平文には一切アクセス不可 | Tink, 各種暗号化ライブラリ |
